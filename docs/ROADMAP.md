...
- **Task 013: 포트폴리오 홈 API 구현 (F005a)** ✅ — 완료 (2026-08-25)
  ...
  - ✅ 취득원가(`cost = quantity × avgPrice`)에 자산유형/통화별 정밀도 규칙 적용 — COIN은 통화 무관 8자리, 그 외엔 통화 기준(KRW 0/USD 2, 2026-09-07 세션에서 4→2로 정정 — 아래 「금융 정밀도 규칙」 표 참고), HALF_UP. Task 011이 남겨뒀던 갭("Task 013 착수 시 서버 응답도 이 규칙을 따라야 한다") 해소
...
- **Task 015: 물타기 시뮬레이터 구현 (F006)** ✅ — 완료 (2026-08-25)
  ...
  - ✅ In-Memory 가중평균 계산, DB 쓰기 없음(`@Transactional(readOnly = true)`, `save`/`flush`/`delete` 호출 0건 — 2차 code-reviewer가 grep으로 실측 확인). `BigDecimal.divide(분모, scale, HALF_UP)`로 나눗셈·반올림을 1회에 처리. scale 결정 로직(`COIN` 8자리, `KRW` 0/`USD` 2/기타 2)은 `PortfolioService.scaleFor`/`scaleForCurrency`를 로컬 복제(2차 code-reviewer가 두 메서드 본문을 문자열로 직접 비교해 완전히 동일함을 확인) — 공유 유틸 추출은 Task 016에서 완료(아래 참고)
...
- **Task 023: 포트폴리오 평가금액·비중·손익 (F005b)** ✅ — 완료 (2026-09-04)
  ...
  - ✅ **[후속] 코인 시세·USD 스케일 정정** (2026-09-07, Task 024 완료 이후 사용자 요청으로 진행된 세션, formal Task 번호 없이 진행)
    ...
    - 코인 캐시 키(`price:COIN:{ticker}`)에 KRW가 아닌 통화를 접미사로 반영(`price:COIN:{ticker}:{currency}`) — 같은 티커를 KRW/USD로 각각 등록한 두 자산이 캐시를 공유해 서로 다른 가격을 덮어쓰는 문제 방지. 기존 KRW 코인의 키는 접미사 없이 그대로 유지(하위 호환)
    - USD 스케일을 사용자 지시로 4→2로 변경(`PrecisionScale.scaleForCurrency`, `frontend/src/lib/money.ts`의 `SCALE.USD`, 이 문서의 「금융 정밀도 규칙」 표 동기화). 자산 상세 화면의 평단가·취득원가·평단가 차트에서 COIN에 한해 후행 0을 트리밍하도록 `formatAmount`를 수정(KRW/USD는 기존 고정 스케일 표시 유지)
    - `code-reviewer` 에이전트 독립 검증 — Blocker 0건, Major 4건 발견해 전부 수정: (1) 마켓 접두어가 이미 포함된 티커(예: `KRW-BTC`)와 자산 `currency`(예: `USD`)가 어긋나면 원화 시세에 환율을 또 곱하는 이중 변환 결함 — 응답 통화를 `currency` 인자가 아니라 실제로 조회한 마켓 접두어에서 도출하도록 `UpbitPriceClient`를 수정 (2) 코인 USD 환율 조회가 CASH(USD)의 12시간 캐시를 우회해 COIN의 10초 freshTtl 주기로 환율 API가 반복 호출되던 문제 — `price:CASH:USD` 캐시를 재사용하도록 수정 (3) 자산 상세 "수정" 폼이 `NUMERIC(28,8)` 원본값을 통화별 표시 스케일(KRW 0/USD 2)로 반올림해 채워 넣어, 사용자가 그대로 제출하면 평단가가 조용히 반올림되는 데이터 손실 결함(`70000.5` → `70001`) — `toEditableAmount()`가 스케일 8로만 트리밍하고 반올림하지 않도록 수정 (4) `StockPriceClient`의 서비스키 디코딩이 항상 `URLDecoder`를 돌려, 공공데이터포털이 발급하는 두 키 형식 중 "디코딩 키"(Base64 원문, 리터럴 `+` 포함 가능)를 넣으면 `+`가 공백으로 깨지는 잠재 회귀(이 세션의 다른 변경이 아니라 이전 세션 잔여 수정 — 코드 리뷰로 발견) — Base64엔 `%`가 절대 안 나온다는 성질을 이용해 `%XX` 패턴이 있을 때만 디코딩하도록 `decodeIfAlreadyEncoded()`로 교체, 두 키 형식 모두 안전
    - 백엔드 신규 테스트 `UpbitPriceClientTest` +3·`PriceServiceTest` +3·`StockServiceKeyDecodingTest`(신규 파일) +3, 프론트 신규 테스트 `money.test.ts`·`PortfolioPage.test.tsx` 각 다수 + `AssetDetailPage.test.tsx` 기존 1건 값 갱신 — `./gradlew test --rerun-tasks`(백엔드 204개)·`npm run test`(프론트 155개) 전체 스위트 재실행 통과
