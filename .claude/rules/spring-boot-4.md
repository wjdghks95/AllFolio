...
| `RestClient` 커넥트/리드 타임아웃 프로퍼티 프리픽스 | Spring Boot 4.1에서 `RestClient.Builder`의 기본 타임아웃을 바인딩하는 `@ConfigurationProperties` 프리픽스는 `spring.http.client`(단수)가 아니라 `spring.http.clients`(복수, `HttpClientsProperties` 바이트코드로 확인)다 — 단수형으로 설정하면 조용히 무시되고 `HttpClientSettings.defaults()`(타임아웃 없음)가 적용돼 외부 API가 응답을 영원히 기다린다(WireMock `withFixedDelay(5000)` 스텁이 `read-timeout: 3s` 설정에도 5초 전부를 기다린 뒤 200으로 성공하는 것으로 실측, `UpbitPriceClientTest` 회귀). `application.yml`에는 반드시 `spring.http.clients.connect-timeout`/`read-timeout`으로 쓸 것 |
