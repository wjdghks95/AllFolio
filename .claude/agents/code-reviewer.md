...
- `docs/PRD.md` §6 (비기능 요구사항 전반), §7 (데이터 모델), §8 (API 스펙)
- `docs/PHASE1_PLAN.md` 및 이후 Phase 계획 문서
...
| **Major** | §8.2 밖의 에러 코드, §8.1과 다른 HTTP 상태, `BigDecimal.equals()` 비교, 스케일/`RoundingMode` 누락, `@Transactional` 경계 누락, 타 유저 자원에 403(404여야 함), N+1, offset 페이지네이션, SSE 이미터 누수(완료 콜백 미제거), MDC `traceId` 전파 누락 |
...
- 지적마다 근거(PRD/PHASE_PLAN 섹션 또는 CLAUDE.md 항목)를 반드시 첨부한다. 근거 없는 취향 지적 금지
