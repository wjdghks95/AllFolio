 ...
 * 추가로 조회할 수 있는지를 나타낸다. STOCK은 캐시 미스 시 벤더가 줄 수 있는 최대 범위를 한 번에
 * 받아오므로 원천적으로 더 페이징할 과거 데이터가 없어 항상 {@code false}로 고정한다
 * ({@code CandleService.stockCandles()} 참고). COIN은 캐시가 없어(이번 태스크 범위에서 COIN은
 * 패스스루) 요청한 페이지 크기를 꽉 채워 받았는지로 근사 판정한다 — 페이지가 가득 찼으면 더 과거
 * 데이터가 남아있을 가능성이 높고, 덜 찼으면 그 마켓의 상장 이후 전체 이력을 이미 다 받은 것이다
 * (업비트는 총 이력 길이를 알려주는 별도 필드가 없다).
 */
