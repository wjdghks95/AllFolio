...
## Redis 락은 소유권 토큰을 호출자가 들고 있어야 한다 (Task 028 실측)

새 Redis 기반 락 컴포넌트(`CandleRangeLock`)를 만들 때 소유권 토큰을 인-프로세스 `Map`(락키→토큰)에
저장했다가, 같은 키를 두 번째로 획득하면 그 맵이 이전 토큰을 덮어쓰는 결함이 실제로 재현됐다 — TTL
만료로 이미 새 보유자가 같은 키를 잡은 상태에서 원래 보유자가 자기 `finally`의 `unlock()`을 호출하면
맵에서 "새 보유자의 토큰"을 꺼내 그 락을 지워버린다. `tryLock`이 획득한 토큰을 호출자에게 반환하고,
`unlock(key, token)`으로 그 토큰을 정확히 받아야만 지우게 할 것 — 호출자(스택 프레임)마다 자기 토큰을
따로 들고 있으므로 인-프로세스 공유 상태가 없어진다. 새 Redis 락을 추가할 때도 이 패턴을 따를 것.
