...
class CandleSseStreamIntegrationTest extends AbstractIntegrationTest {
    ...
    void subscribingViaQueryTokenReceivesCandleUpdateAfterPollAndDeduplicatesUnchangedTicks() throws Exception {
        ...
        // 전송 스레드 로그에 traceId·userId가 둘 다 채워졌는지 실측(MdcPropagation + emitter별 userId 전파 검증).
        ...
        assertThat(sendLogEvent.getMDCPropertyMap().get("userId")).isNotBlank();
    ...
    /**
     * SecurityConfig의 {@code DispatcherType.ASYNC}용 {@code permitAll()}(Task 028)이 REQUEST
     * 디스패치의 인가까지 약화시키지 않았는지 회귀로 고정한다(code-reviewer m10) — 이 permitAll은
     * 전역 ASYNC 디스패치에 적용되므로, 일반 REQUEST 엔드포인트가 여전히 401을 내는지 별도로
     * 확인해야 한다.
     */
    @Test
    void regularEndpointStillRequiresAuthenticationAfterAsyncPermitAll() throws Exception {
        HttpRequest request = HttpRequest.newBuilder()
